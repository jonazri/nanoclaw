...
export async function updateVoiceProfile(
  ...
  // Weight the existing embedding by its sample count so historical
  // samples are not diluted by a small number of new samples.
  const n = existingProfile.sampleCount;
  const dim = existingProfile.embedding.length;
  const totalSamples = n + newEmbeddings.length;
  const weightedSum = existingProfile.embedding.map((v) => v * n);
  for (const emb of newEmbeddings) {
    for (let i = 0; i < dim; i++) {
      weightedSum[i] += emb[i];
    }
  }
  const avg = weightedSum.map((v) => v / totalSamples);
  const norm = Math.sqrt(avg.reduce((s, v) => s + v * v, 0));
  const averaged = avg.map((v) => v / norm);
